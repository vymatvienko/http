...
export default class TicketService {
  ...
  async init() {
    ...
    for (let i = 0; i < this.changeButtons.length; i++) {
      const changeButton = this.changeButtons[i];
      changeButton.addEventListener('click', this.changeTicketForm.bind(this));
    }
    for (let i = 0; i < this.deleteButtons.length; i++) {
      const deleteButton = this.deleteButtons[i];
      deleteButton.addEventListener('click', this.deleteTicketForm.bind(this));
    }
    for (let i = 0; i < this.tickets.length; i++) {
      const ticket = this.tickets[i];
      ticket.addEventListener('click', this.showTicketDescription.bind(this));
    }
    for (let i = 0; i < this.statusButtons.length; i++) {
      const statusButton = this.statusButtons[i];
